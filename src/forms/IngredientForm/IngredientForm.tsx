...
import { CreateIngredientCommand, UpdateIngredientCommand } from "src/generatedAPI";
...
const IngreidentForm = ({
  ...
  const onSubmitHandler: SubmitHandler<FormValues> = async (data: FormValues) => {
    const { name } = data;
    const { data: { results } } = await api.tags.getEquipment({
      hidden: false,
      promoted: false,
      search: name
    });
    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmitHandler)}>
        ...
        <FormTextField
          name="icon"
          fullWidth
          label={t("types.ingredient.fields.icon.name")}
        />
